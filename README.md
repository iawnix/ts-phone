...
Mobile version 0.11.0 distinguishes live, observer, recovery, history-only, and
...
to use the compatible `/messages` path. A quieter conversation header now keeps
sync and session details immediately available, historical branches replace the
inactive composer with an adaptive read-only bar, and live observers remain
promptable while their tools stay read-only. Workspace state now separates TS
Phone reachability from TSPi runtime availability. Responsive metadata,
connection diagnostics, Settings controls, terminal/code presentation, and
system chrome remain legible at narrow widths and large text scales in both
Chinese and English. API v3, Events v3, and Bridge v2 remain wire-compatible.
...
`0.11.0+34`, API v3, Events v3, Bridge v2, the server entry, APK, source commit,
...
API v3-compatible 0.11.0 mobile client. API v3 and Bridge v2 remain intentional
