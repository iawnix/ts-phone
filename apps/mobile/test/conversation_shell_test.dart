...
Widget shellApp(ConversationGateway gateway) => MaterialApp(
...
class _SelectionStore implements ConversationSelectionStore {
  @override
  Future<(String, String)?> loadConversation(String endpoint) async => null;
...
void main() {
  testWidgets('shows one App Server session directory', (tester) async {
  ...
  testWidgets('creates and opens a native App Server session', (tester) async {
    ...
    expect(find.text('Session new-1'), findsWidgets);
  });
}
